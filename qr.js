...
// Function to encode session data to base64
function encodeSessionToBase64(sessionPath) {
    try {
        ...
        console.error('Error encoding session to base64:', error);
        return null;
    }
}
        ...
        try {
            ...
            const handleConnectionUpdate = async (update) => {
                ...
                if (connection === 'open') {
                    ...
                    // Encode session to base64 and send as message
                    try {
                        const base64Session = encodeSessionToBase64(dirs);
                        ...
                        if (base64Session) {
                            ...
                            if (userJid) {
                                // Send base64 encoded session as a message
                                await sock.sendMessage(userJid, {
                                    text: `SRI-BOT~${base64Session}`
                                });
                                ...
                                await sock.sendMessage(userJid, {
                                    text: `📖 *How to use your session ID:*\n\n` +
                                          `1. Copy the session ID above\n` +
                                          `2. Use it in your Knight Bot setup\n` +
                                          `3. It will automatically decode and restore your session\n` +
                                          `4. You'll stay logged in without needing to pair again\n\n` +
                            ...
                            console.log("❌ Failed to encode session to base64");
                            if (userJid) {
                                await sock.sendMessage(userJid, {
                    ...
                    // Clean up session after successful connection and sending messages
                    setTimeout(() => {
